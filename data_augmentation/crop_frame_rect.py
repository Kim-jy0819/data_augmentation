...
num_of_crops = 5 # 프레임당 크롭 수
crop_count = 0

aug_num = 3 # 크롭 당 aug 개수(brightness, ch.shift)
aug_count = 0
...
for i in range(len(path)):
    ...
    while crop_count < num_of_crops and crop_mode == 1 :
        ...
        small = min(mid_x - w_w/2, mid_y - h_h/2, x_orig - (mid_x + w_w/2), y_orig - (mid_y + h_h/2))+4
        # small = min(r / 2, r * 1.5)
        ...
        rand_shift_x=np.random.randint(-int(small/2),int(small/2))
        rand_shift_y=np.random.randint(-int(small/2),int(small/2))
